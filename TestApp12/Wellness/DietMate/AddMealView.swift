...
struct AddMealView: View {
    ...
    private var entry: MealEntry? {
        dietMateStore.mealEntries.first(where: { $0.id == mealEntryId })
    }
    ...
    private var suggestedFoods: [Food] {
        ...
        let all = dietMateStore.foods(for: e.mealType)
    ...
    private func addFood(_ food: Food) {
        let res = dietMateStore.addOrIncrementFood(food, to: mealEntryId)
        // Show ActionResult feedback if needed
    }

    private func incrementFood(_ mealFood: MealFood) {
        dietMateStore.incrementFood(mealFoodId: mealFood.id, mealEntryId: mealEntryId)
    }

    private func decrementFood(_ mealFood: MealFood) {
        dietMateStore.decrementOrRemoveFood(mealFoodId: mealFood.id, mealEntryId: mealEntryId)
    }
}
