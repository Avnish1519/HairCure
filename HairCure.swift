...
struct HairCurePrimaryButton: ViewModifier {
...
struct HairCureSecondaryButton: ViewModifier {
    func body(content: Content) -> some View {
        content
            .font(.system(size: 17, weight: .semibold))
...
extension View {
    func hairCurePrimaryButton() -> some View { modifier(HairCurePrimaryButton()) }
    func hairCureSecondaryButton() -> some View { modifier(HairCureSecondaryButton()) }
}

// MARK: - Input Field Style

struct HairCureInputField: ViewModifier {
    func body(content: Content) -> some View {
        content
            .font(.system(size: 16))
...
extension View {
    func hcInputField() -> some View { modifier(HairCureInputField()) }
}

// MARK: - Progress Bar (dashed segments)

struct HairCureProgressBar: View {
...
struct HairCureBackButton: View {
    let action: () -> Void
