...
final class MindEaseDataStore {
    ...
    private func seedHistoricalMindfulData(userId: UUID, userPlans: [UserPlan]) {
        guard let firstMeditationContent = mindEaseCategoryContents.first(where: { content in
            mindEaseCategories.first { $0.id == content.categoryId }?.title == "Meditation"
        }) else { return }
        ...
        for (daysAgo, fraction) in dayData {
            ...
            let dayStart  = calendar.startOfDay(for: pastDate)
            let startTime = calendar.date(byAdding: .hour, value: 7, to: dayStart) ?? dayStart
            let minutes   = Int((target * fraction).rounded())
            let endTime   = calendar.date(byAdding: .minute, value: minutes, to: startTime) ?? startTime
            mindfulSessions.append(MindfulSession(
                id: UUID(), userId: userId,
                contentId: firstMeditationContent.id,
