            ...
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(entry.mealType.displayName)
                        .font(.system(size: 20, weight: .bold))
                        .foregroundStyle(entry.hasCalories ? entry.mealType.accentColor : .primary)
        ...
        if entry.hasCalories && !isPast {
            Button(action: onEdit) {
                Image(systemName: "square.and.pencil")
                    .font(.system(size: 18)).foregroundStyle(entry.mealType.accentColor)
            }
        } else if entry.hasCalories {
            Text("History")
                .font(.system(size: 11, weight: .medium))
                .foregroundStyle(entry.mealType.accentColor)
            ...
            ForEach(foods, id: \.mealFood.id) { pair in
                Button { onTap(pair.food) } label: {
                    HStack {
                        ...
                        Text(pair.food.name)
                            .font(.system(size: 14)).foregroundStyle(.primary).lineLimit(2)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .lastTextBaseline, spacing: 2) {
                ...
                Text("/\(Int(entry.calorieTarget)) kcal").font(.system(size: 15)).foregroundStyle(.secondary)
            }
            CalorieProgressBar(consumed: entry.caloriesConsumed, target: entry.calorieTarget,
    ...
    var body: some View {
        ...
        HStack(spacing: 10) {
            Image(systemName: style.icon)
                .font(.system(size: 18, weight: .semibold)).foregroundStyle(style.color)
                ...
                HStack(spacing: 12) {
                    ...
                    HStack(spacing: 12) {
                        Button {
                            ...
                            Image(systemName: "minus.circle").font(.system(size: 22)).foregroundStyle(.secondary)
                        }
                        Text("\(Int(pair.mealFood.quantity))")
                        ...
                        Button {
                            ...
                            Image(systemName: "plus.circle").font(.system(size: 22)).foregroundStyle(mealColor)
                        }
                    }
                }
                .padding(12)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Button(action: onTap) {
                Text(food.name)
                    .font(.system(size: 13, weight: .medium))
                    .lineLimit(2).foregroundStyle(.primary).padding(.horizontal, 2)
            }
        }
    }
}
                ...
                VStack(alignment: .leading, spacing: 20) {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Nutrition information")
                            .font(.system(size: 17, weight: .semibold)).foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(spacing: 0) {
            ForEach(Array(nutrients.enumerated()), id: \.offset) { i, name in
                HStack {
                    Image(systemName: "info.circle.fill").font(.system(size: 16)).foregroundStyle(.secondary)
                    Text(name).font(.system(size: 16))
                    Spacer()
                    Image(systemName: "checkmark.circle.fill")
                        .font(.system(size: 22))
                        .foregroundStyle(Color(red: 0.15, green: 0.76, blue: 0.37))
...
#Preview {
    ...
    dietMateStore.addAll(userId: appStore.currentUserId, nutritionProfile: nil)
